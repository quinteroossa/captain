#!/usr/bin/env python
"""Train ecologically-grounded CVaR-PPO with stochastic disturbance (RQ3 v2).

Extends Tang et al. (2020) "Worst Cases Policy Gradients" by decoupling the
CVaR filter criterion from the reward signal, following the constrained RL
tradition of Achiam et al. (2017) CPO and Agnihotri et al. (2024) e-COP.

Key difference from train_ppo_cvar.py (v1):
  v1 filters episodes by TOTAL REWARD — conflates ecological value and cost.
  v2 filters episodes by ECOLOGICAL SCORE:
      eco_score = -(NT×1 + VU×8 + EN×16 + CR×32)
  Worst episodes = lowest ecological score = most threatened species.
  PPO update still uses the standard reward gradient (unchanged).

This ensures CVaR directly captures worst-case extinction outcomes rather
than worst-case reward, which can be driven by cost variance. The
formulation addresses the gap identified in conservation RL (Lapeyrolerie
et al. 2022; Equihua et al. 2024): existing methods optimise expected
outcomes and ignore worst-case disturbance scenarios.
...
Usage:
    uv run python experiments/train_ppo_cvar_v2.py \\
        --config experiments/configs/ppo_cvar_v2.yaml \\
        --data-dir /path/to/captain3data \\
        --run-name ppo_cvar_v2 \\
        --wandb
"""
    ...
    for update in range(cfg["n_updates"]):
        ...
        with torch.no_grad():
            for _ in range(n_episodes):
                ...
                # Compute ecological score from final species counts:
                # lower score = worse ecological outcome = more threatened species
                ...
                episodes.append((ep_return, eco_score, steps, info))

        # ------------------------------------------------------------------
        # Phase 2: Filter to worst alpha fraction by ECOLOGICAL SCORE
        # Episodes with the lowest eco_score had the most threatened species.
        # PPO update uses reward-based advantages (unchanged).
        # ------------------------------------------------------------------
        episodes.sort(key=lambda x: x[1])  # ascending: worst ecology first
        worst    = episodes[:n_keep]
        all_rets = [ep[0] for ep in episodes]
        cvar_value  = float(np.mean([ep[1] for ep in worst]))  # mean eco_score of worst
        ...
        for _, _eco, steps, info in worst:
        ...
        if args.wandb:
            wandb_data = {
                ...
                "ecology/cvar_score": cvar_value,  # mean eco_score of worst-alpha episodes
                "loss/policy":      mean_pl,
                "loss/value":       mean_vf,
